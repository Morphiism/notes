...
满足以上性质的函数称为度量或者距离函数。度量空间中的元素称为点。
...
此外，度量空间 $X$ 的任意子集 $Y$ 也是度量空间，我们可以验证函数 $d$，限制在 $Y$ 上，满足定义中的三条性质。因此，euclidean 空间的任意子集也是度量空间。其他例子包括紧空间上的连续函数空间 $C(K)$ 以及可积函数空间 $L^{1}(\mu)$。
...
$$
\begin{gather}
\left( \frac{3k+1}{3^{m}},\frac{3k+2}{3^{m}} \right) \tag{2.48.1}
\end{gather}
$$

的开区间与 $P$ 相交。事实上，$P$ 不包含任何开区间，因为每个开区间 $(\alpha,\beta)$ 都包含一个形如 $(2.48.1)$ 的区间，其中
